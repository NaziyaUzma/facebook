$( document ).ready(() => {
    ...
    function getFacebookPosts(start,end)
    {
        ...
        $.ajax({
            type:'GET',
            dataType:'JSON',
            url:'https://graph.facebook.com/me?fields=posts{created_time,type,full_picture,story,message,source,likes,comments,shares,actions},name&access_token='+myFacebookToken,
            ...
            success : (response) => 
            {
                    ...
                    $.each(myPosts, (i, item) =>
                    {
                        ...
                        if(myPosts[i].likes != undefined){
                            myLikes[i] = myPosts[i].likes.data[0].name + " and " + (myPosts[i].likes.data.length -1)  + " more";    
                        }
                        else{
                            myLikes[i] = "";          
                        }
                            ...
                            "<div class='container ' id='postCard'>" +
                                ...
                                "<div class='row no-gutters'>" +
                                    "<div class='col-lg-auto'> " +
                                        "<span id='myActionLikes'><a title='Like' href='" + myPosts[i].actions[0].link + "'><i class='fas fa-thumbs-up'></i></a></span>"+
                                        "<span id='myActionComments'><a title='Comment' href='" + myPosts[i].actions[0].link + "'> <i class='fas fa-comment'></i></a></span>"+
                                        "<span id='myActionShares'><a title='Share' href='" + myPosts[i].actions[0].link + "'> <i class='fas fa-share'></i></a></span>"+
                                        //"<span id='myActionShares'> <a href <i class='fas fa-share'></i></span>"+
                ...
                { // error function for showing the error on console and giving warining to users via alert
                    $('#loaderPosts').delay(1000).hide(); // hide the loader on screen                    
    ...
    $("#getCount").on('click',() => {
	start=end;
        end = (end+4 <= totalPosts) ? end+4 : totalPosts;
